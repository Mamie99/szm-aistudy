...
atexit.register(close_db, db_conn)     # 注册进程退出时的清理钩子hook ,只要不是断电、kill -9都可以自动关闭数据库资源
...
def get_employee_profile(uid:str) -> str:
    ...
    if not res:
        return f"「错误」︰未找到UID 为 {uid} 的员工信息"

    employee = res[0]
    return (
        f'[档案查询结果] 员工姓名: {employee[ "name"]}，职级:{employee["rank"]},'
...
def get_leave_balance(uid:str) -> str:
    ...
    """
    sql = """
    select a.name, b.annual_leave_remaining, b.sick_leave_remaining
    from employees a LEFT JOIN leave_balance b on a.uid = b.uid
    where a.uid = ?
    """

    res = query_db(conn=db_conn, sql=sql, params=(uid,))
    if not res:
        return f"「错误」︰未找到UID 为 {uid} 的假期数据"

    data = res[0]
    return (
        f'[剩余假期] 员工: {data[ "name"]}（UID: {uid}），剩余年假:{data["annual_leave_remaining"]}天,'
        f'剩余病假: {data["sick_leave_remaining"]}天'
    )

@tool
def generate_employment_certificate(uid:str, cer_type:str):
    """
    为指定员工自动生成证明文件。
    参数 cer_type 必须是以下两个值之—:
    - ' employment':仅开具在职证明(全员可用)
    - 'income ':开具包含薪资的在职及收入证明（有职级权限限制，仅P5及以上可用)
    """
    sql = """select name, rank, base_salary from employees where uid = ?"""
    rmp_res = query_db(conn=db_conn, sql=sql, params=(uid,))
    ...
    if cer_type == 'income':
        ...
        if rank_level < 5:
            return (
                f'[系统提示] 根据公司规定，P4 及以下职级员工（当前员工为{employee["rank"]}），无法线上开具薪资收入证明'
                f'请引导员工在线提交人工工单，由 hr 线下手动核实开具'
            )
        content = (
            f'《薪资收入证明》\n 兹证明：我司员工 {employee["name"]}，职级为：{employee["rank"]}，\n'
        ...
        content = (
            f'《在职证明》\n 兹证明：我司员工 {employee["name"]}，现为我司在职职员，职级为 {employee["rank"]}。特此证明'
            f'\n （公章）'
        )
        return (
            f'[系统提示] 已为您自动生成在职证明：------------\n'
